...
```{r camp-outcome}
lead_snp_camp <- "X5.149061758.C.T_C"   # rs36047
camp_df <- camp_df |>
...
camp_snp_outcome <- glm(
  outcome ~ `X5.149061758.C.T_C` + AGE.x + SEX.x +
    PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10,
  data = camp_df,
  family = binomial()
)

summary(camp_snp_outcome)
broom::tidy(camp_snp_outcome, exponentiate = TRUE, conf.int = TRUE)
```
...
summary(camp_med_fit)
broom::tidy(camp_med_fit)
```

## CAMP miRNA -> exacerbation outcome model with interaction

```{r camp-outcome-model-interaction}
camp_out_fit_int <- glm(
  outcome ~ `X5.149061758.C.T_C` + miRNA584_norm * treatment_grp +
    AGE.x + SEX.x +
    PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10,
  data = camp_df,
  family = binomial()
...
summary(camp_out_fit_int)
broom::tidy(camp_out_fit_int, exponentiate = TRUE, conf.int = TRUE)
```
